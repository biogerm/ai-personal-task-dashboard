...
## Rules
- When casting to Nest Hub, always ensure that a health check is in place (e.g. `catt info`) to verify the hub is still in cast mode (e.g. DashCast is active). If the cast drops, it must be automatically restarted.
- After making any UI or backend modification that affects the dashboard, always run `catt cast_site` again to force the Nest Hub to refresh.
