...
function processLeftWidgetTasks(tasks, isLockScreen) {
  const now = new Date();
  ...
  if (isLockScreen) {
    ...
    for (let t of tasks) {
      ...
      if (!dateStr) {
        ...
        if (dateOnly <= localToday) pastAndToday.push(t);
      }
    }
    pastAndToday.sort((a, b) => {
      let dA = a.properties["Due Date"].date.start.substring(0,10);
      let dB = b.properties["Due Date"].date.start.substring(0,10);
    ...
    return { lockScreenTasks: [...pastAndToday, ...noDue] };
    ...
    for (let t of tasks) {
      ...
      if (!dateStr) {
        ...
        let dateOnly = dateStr.substring(0, 10);
        if (dateOnly < localToday) pastDue.push(t);
        else if (dateOnly === localToday) today.push(t);
        else other.push(t);
      }
    }
    ...
    pastDue.sort(sortPriorityDate);
    today.sort(sortPriorityDate);
    other.sort(sortPriorityDate);
    return { pastDue, today, other };
  }
}
...
async function createWidget(tasksData, isLockScreen) {
  ...
  let isEmpty = isLockScreen 
    ? tasksData.lockScreenTasks.length === 0 
    : (tasksData.pastDue.length + tasksData.today.length + tasksData.other.length === 0);
  ...
  if (isLockScreen) {
    await renderSection("", tasksData.lockScreenTasks);
  } else {
    await renderSection("PAST DUE", tasksData.pastDue);
    await renderSection("TODAY", tasksData.today);
    await renderSection("OTHER", tasksData.other);
  }
...
const tasksData = processLeftWidgetTasks(tasks, isLockScreen);
const widget = await createWidget(tasksData, isLockScreen);
