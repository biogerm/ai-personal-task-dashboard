...
function render(data) {
    ...
    if (!data.projects || data.projects.length === 0) {
        projectsList.innerHTML = '<li class="empty-state">' + t("ui.no_active") + '</li>';
    } else {
        const todayProjects = data.projects.filter(p => p.urgency === 'today');
        const overdueProjects = data.projects.filter(p => p.urgency === 'overdue');
        const allOtherProjects = data.projects.filter(p => p.urgency !== 'today' && p.urgency !== 'overdue');
        ...
        if (overdueProjects.length > 0) {
            ...
            projectsList.appendChild(header);
            overdueProjects.forEach(p => projectsList.appendChild(renderProject(p)));
        }
        ...
        if (allOtherProjects.length > 0) {
            ...
            for (const project of allOtherProjects) {
                ...
                const isOverflowing = projectsList.scrollHeight > projectsList.clientHeight;
            ...
            if (hiddenOthersCount > 0) {
                ...
                moreIndicator.textContent = t("ui.more_projects", {count: hiddenOthersCount});
    ...
    switch (project.urgency) {
        ...
        case 'upcoming':
            if (project.due_date) {
                ...
                dateText = due.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
            }
...
function updateSyncStatus(data) {
    ...
    if (!data) {
        statusSpan.textContent = t("ui.offline", {time: lastKnownTime || t("ui.unknown")});
    ...
    if (errorSources.length > 0) {
        statusSpan.textContent = t("ui.sync_failed", {sources: errorSources.join(', ')});
        statusSpan.classList.add('offline');
    } else if (isPending) {
